...
export default function TestPage({ params }: { params: { id: string } }) {
  ...
  const updateQuestion = useCallback(async (question: Question) => {
    ...
    await axios.put(`/api/tests/${params.id}`, {
      questions: test.questions,
      notes
    })
  }, [params.id, notes, test?.questions])
  ...
  const handleEndTest = async () => {
    ...
    router.push(`/test/${params.id}/summary`)
  }
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1 bg-white p-6 rounded-xl shadow-sm">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-xl font-semibold">سوال {currentQuestion + 1}</h2>
            </div>
                ...
                {[1, 2, 3, 4].map(option => (
                  <button
                    ...
                    className={`p-4 rounded-lg ${
                      test.questions[currentQuestion].selectedOption === option
